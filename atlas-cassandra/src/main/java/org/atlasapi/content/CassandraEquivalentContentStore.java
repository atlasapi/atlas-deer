...
public class CassandraEquivalentContentStore extends AbstractEquivalentContentStore {
    ...
    private void deleteStaleRows(EquivalenceGraph updated, ImmutableSet<EquivalenceGraph> created) {
        ImmutableList<Statement> statements = created.stream()
                .flatMap(graph -> deleteStaleGraphs(updated, graph))
                .collect(ImmutableCollectors.toList());

        executeAsync(statements, writeConsistency);
    }

    private Stream<Statement> deleteStaleGraphs(EquivalenceGraph updatedGraph,
            EquivalenceGraph createdGraph) {
        return createdGraph.getEquivalenceSet().stream()
                .map(id -> deleteStaleGraph(updatedGraph, id));
    }

    private Statement deleteStaleGraph(EquivalenceGraph updatedGraph, Id contentId) {
        return delete()
                .from(EQUIVALENT_CONTENT_TABLE)
                .where(eq(SET_ID_KEY, updatedGraph.getId().longValue()))
                .and(eq(CONTENT_ID_KEY, contentId.longValue()));
    }

    private void deleteStaleSets(Set<Id> deletedGraphs) {
        ImmutableList<Statement> statements = deletedGraphs.stream()
                .map(this::deleteStaleSet)
                .collect(ImmutableCollectors.toList());

        executeAsync(statements, writeConsistency);
    }

    private Statement deleteStaleSet(Id id) {
        return delete().all()
                .from(EQUIVALENT_CONTENT_TABLE)
                .where(eq(SET_ID_KEY, id.longValue()));
    }

    private void updateDataRows(ImmutableSetMultimap<EquivalenceGraph, Content> graphsAndContent) {
        ImmutableList<Statement> statements = graphsAndContent.entries().stream()
    ...
    private void executeAsync(Iterable<Statement> statements, ConsistencyLevel consistency) {
        ImmutableList<ResultSetFuture> resultSetFutures = StreamSupport
                .stream(statements.spliterator(), false)
                .map(statement -> statement.setConsistencyLevel(consistency))
                .map(session::executeAsync)
                .collect(ImmutableCollectors.toList());

        resultSetFutures.forEach(ResultSetFuture::getUninterruptibly);
    }
}
