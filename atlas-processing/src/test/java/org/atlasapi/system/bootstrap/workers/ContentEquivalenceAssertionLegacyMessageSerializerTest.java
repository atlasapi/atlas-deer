...
public class ContentEquivalenceAssertionLegacyMessageSerializerTest {
    ...
    public void testDeSerializesContentEquivalenceAssertionMessage() throws Exception {
        ImmutableList<AdjacentRef> adjs = ImmutableList.of(new AdjacentRef("cf2",
                "item",
                Publisher.PA.key()));
        ...
        assertThat(
                deserialized.getSubject().getId().longValue(),
                is(SubstitutionTableNumberCodec.lowerCaseOnly()
                        .decode(msg.getEntityId())
                        .longValue())
        );
        ...
        assertThat(
                Iterables.getOnlyElement(deserialized.getAssertedAdjacents()).getId().longValue(),
                is(SubstitutionTableNumberCodec.lowerCaseOnly()
                        .decode(Iterables.getOnlyElement(adjs).getId())
                        .longValue())
        );
        assertThat(
                Iterables.getOnlyElement(deserialized.getAssertedAdjacents()).getSource().key(),
                is(Iterables.getOnlyElement(adjs).getSource())
        );
        assertThat(
                Iterables.getOnlyElement(deserialized.getAssertedAdjacents()),
                instanceOf(ItemRef.class)
        );
