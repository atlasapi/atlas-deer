...
public class ChannelDayScheduleBootstrapTaskTest {
    ...
    public void testRunningTask() throws Exception {
        ...
        ChannelIntervalScheduleBootstrapTask task
                = new ChannelIntervalScheduleBootstrapTask(
                resolver,
                writer,
                contentStore,
                src,
        ...
        when(contentStore.resolveIds(ImmutableSet.of(item1.getId(), item2.getId())))
                .thenReturn(Futures.immediateFuture(Resolved.valueOf(ImmutableSet.<Content>of(
                        item1,
                        item2
                ))));
        when(contentStore.resolveIds(ImmutableSet.of(
                brand1.getId(),
                brand2.getId(),
                series2.getId()
        )))
