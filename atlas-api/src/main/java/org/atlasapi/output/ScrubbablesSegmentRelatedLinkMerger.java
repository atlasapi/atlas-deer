...
/**
 * This is a client-specific strategy for merging together and ordering the set of {@link
 * RelatedLink} from a set of {@link org.atlasapi.segment.Segment} and {@link
 * org.atlasapi.segment.SegmentEvent}.
 */
public class ScrubbablesSegmentRelatedLinkMerger implements SegmentRelatedLinkMerger {
     ...
     * RelatedLinks found on each Segment. For each Segment & Event pair in the map, the interval
     * defined by their offset and duration is compared to the 'selected' pair's. Intervals that
     * overlap the most with the selected Segment & Event are ordered higher. Pairs that do not
     * overlap do not have their RelatedLinks merged in.
     *
     ...
     */
    @Override
    public ImmutableList<RelatedLink> getLinks(Segment selectedSegment,
            SegmentEvent selectedSegmentEvent,
    ...
    private Ordering<Map.Entry<Segment, SegmentEvent>> greatestOverlapDurationOrdering(
            final Interval selectedInterval) {
        return new Ordering<Map.Entry<Segment, SegmentEvent>>() {

            @Override
            public int compare(Map.Entry<Segment, SegmentEvent> left,
                    Map.Entry<Segment, SegmentEvent> right) {
                return overlapDurationOf(
                        selectedInterval,
                        intervalFrom(right.getKey(), right.getValue())
                )
                        .compareTo(
                                overlapDurationOf(
                                        selectedInterval,
                                        intervalFrom(left.getKey(), left.getValue())
                                ));
            }
        };
    }
     ...
     */
    private Predicate<Map.Entry<Segment, SegmentEvent>> overlappingIntervalPredicate(
            final Interval selectedInterval) {
