...
public abstract class AbstractSourceGenerator<TI extends TypeInfo, MI extends MethodInfo> implements SourceGenerator<TI, MI> {
	...
	private String generateImports() {
		...
		for (Class<?> importedClass : importedClasses()) {
			imports.append(String.format(IMPORT_DECLARATION, importedClass.getCanonicalName().toString()));
		}
		return imports.toString();
	}
	
	private String generateClassDeclaration(TI typeInfo) {
		return String.format(CLASS_DECLARATION, typeInfo.className(), inheritedInterfaceName().getSimpleName().toString());
	}
	...
	private String generateMethodBasedFields(Iterable<MI> methodsInfo) {
		...
		methodBasedFields.append(formatOverriddenMethod(setName(), String.format("Set<%s>", setType().getSimpleName().toString()), setName()));
		
		return methodBasedFields.toString();
	}
	
	private String createSetFromMethods(String fieldInfoSetName, Class<?> setType, Iterable<MI> methodsInfo) {
		StringBuilder setFromMethods = new StringBuilder();
		
		setFromMethods.append(String.format("%3$sprivate static final Set<%2$s> %1$s = ImmutableSet.<%2$s>builder()\n", fieldInfoSetName, setType.getSimpleName().toString(), TAB));
		
		for (MI method : methodsInfo) {
			// TODO this is vomit-worthy
			// get rid of these darn tabs 
			setFromMethods.append(String.format("%2$s.add(\n%1$s\n%2$s)\n", setElementFromMethod(method, TAB + TAB + TAB + TAB), TAB + TAB + TAB));
		}
	...
	protected String formatOverriddenMethod(String methodName, String returnType, String returnValue) {
	...
	/**
	 * Each imported class will be added to the file in the form:
	 * <code>import <fully qualified class name>;\n</code>
	 */
