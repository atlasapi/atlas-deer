...
public class LegacyOrganisationResolver implements OrganisationResolver {
    ...
    public LegacyOrganisationResolver(OrganisationStore store,
            LegacyOrganisationTransformer transformer) {
        this.store = store;
        this.transformer = transformer;
    }

    @Override
    public ListenableFuture<Resolved<Organisation>> resolveIds(
            Iterable<Id> ids) {
