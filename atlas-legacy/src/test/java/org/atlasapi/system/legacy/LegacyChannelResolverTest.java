...
public class LegacyChannelResolverTest {
    ...
    @Test
    public void testResolveIdsWithGenre() throws Exception {
        ...
        Iterable<Channel> result = Futures.get(
                Futures.transform(objectUnderTest.resolveIds(ids, Optional.of(filterGenre)), new Function<Resolved<Channel>, Iterable<Channel>>() {
                    @Override
        ...
        assertThat(Lists.newArrayList(result), containsInAnyOrder(channel1, channel3));
    }


    @Test
    public void testResolveIdsWhenNoGenre() throws Exception {
        Id id1 = Id.valueOf(1);
        Id id2 = Id.valueOf(2);
        Id id3 = Id.valueOf(3);
        ...
        Iterable<Channel> result = Futures.get(
                Futures.transform(objectUnderTest.resolveIds(ids, Optional.<String>absent()), new Function<Resolved<Channel>, Iterable<Channel>>() {
                    @Override
                    public Iterable<Channel> apply(Resolved<Channel> input) {
                        return input.getResources();
                    }
