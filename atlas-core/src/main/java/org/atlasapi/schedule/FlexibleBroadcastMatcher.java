...
/**
 * <p>Determines whether two {@link Broadcast}s share a schedule slot to within a certain
 * transmission time flexibility.</p>
 * <p>
 * Broadcasts share a schedule slot if: <ul> <li>they are on the same channel</li> <li>start at the
 * same instant ± flexibility</li> <li>(optionally) end at the same instance ± flexibility</li>
 * <ul>
 */
public class FlexibleBroadcastMatcher {
    ...
    public Optional<Broadcast> findMatchingBroadcast(Broadcast subject,
            Iterable<Broadcast> objects) {
    ...
    private boolean startTimeMatch(Broadcast subject, Broadcast object) {
        return withInFlexibility(
                subject.getTransmissionTime(),
                startFlexibility,
                object.getTransmissionTime()
        );
    }
    ...
    private boolean endTimeMatch(Broadcast subject, Broadcast object) {
        return !endFlexibility.isPresent() ||
                withInFlexibility(
                        subject.getTransmissionEndTime(),
                        endFlexibility.get(),
                        object.getTransmissionEndTime()
                );
    }
